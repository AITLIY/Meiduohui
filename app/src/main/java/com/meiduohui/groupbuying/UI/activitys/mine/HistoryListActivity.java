            ...
            public boolean onMenuItemClick(final int position, SwipeMenu menu, final int index) {
                switch (index) {
                    case 0:
                        // 删除
                        mLoadingDailog.show();
                        historyDel(mShowList.get(position).getId());
                        break;
                }
                return false;
            }
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.POST, url, new Response.Listener<String>() {
            ...
            public void onResponse(String s) {
                if (!TextUtils.isEmpty(s)) {
                    LogUtils.i(TAG + "getHistoryList result " + s);

                    try {
                        JSONObject jsonResult = new JSONObject(s);
                        String msg = UnicodeUtils.revert(jsonResult.getString("msg"));
                        LogUtils.i(TAG + "getHistoryList msg " + msg);
                        String status = jsonResult.getString("status");

                        if ("0".equals(status)) {

                            String data = jsonResult.getString("data");
                            mHistoryBeans = new Gson().fromJson(data, new TypeToken<List<HistoryBean>>() {
                            }.getType());

                            mHandler.sendEmptyMessage(LOAD_DATA1_SUCCESS);
                            LogUtils.i(TAG + "getHistoryList mHistoryBeans.size " + mHistoryBeans.size());
                        }
            ...
            public void onErrorResponse(VolleyError volleyError) {
                LogUtils.e(TAG + "getHistoryList volleyError " + volleyError.toString());
            ...
            protected Map<String, String> getParams() throws AuthFailureError {
                ...
                String token = HttpURL.MEM_HISTORYLIST + TimeUtils.getCurrentTime("yyyy-MM-dd") + CommonParameters.SECRET_KEY;
                LogUtils.i(TAG + "getHistoryList token " + token);
                ...
                LogUtils.i(TAG + "getHistoryList json " + map.toString());
                return map;
            }
